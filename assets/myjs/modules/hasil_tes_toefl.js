...
$(document).on("click", ".addSertifikatCourse", function () {
	let data = $(this).data("id");
	data = data.split("|");
	console.log(data);
	id = data[0];
	...
	Swal.fire({
		icon: "question",
		text: "Yakin akan menambahkan sertifikat course untuk " + nama + "?",
		showCloseButton: true,
		showCancelButton: true,
		confirmButtonText: "Ya",
		cancelButtonText: "Tidak",
		...
		if (result.value) {
			...
			let result = ajax(
				url_base + "tes/add_sertifikat_toefl_course",
				"POST",
				data
			);
			...
			if (result == 1) {
				...
				Swal.fire({
					position: "center",
					icon: "success",
					text: "Berhasil menambahkan sertifikat course",
					showConfirmButton: false,
					timer: 1500,
				});
			} else {
